...
export default function App() {
  ...
  async function generateAndCopyReverseSetup() {
    setSshSetupBusy(true); setSshSetupError("");
    try {
      const generated = await bridge.call<{ keyId: string; publicKey: string }>("bridge/ssh/key/generate");
      setGeneratedSshKey(generated);
      setSshPrivateKey("");
      await navigator.clipboard.writeText(reverseHostPrepareScript(generated.publicKey));
    } catch (reason) { setSshSetupError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setSshSetupBusy(false); }
  }
  ...
  async function createSshHost() {
    ...
    try {
      const machine = await bridge.call<Machine & { tunnelSetup?: TunnelSetup }>("bridge/ssh/host/create", {
        name: sshName, host: sshConnectionMode === "reverse-ssh" ? "127.0.0.1" : sshHost, port: Number(sshPort), username: sshUsername,
...
function FilePreviewView({ preview, openImage, download }: { preview: FilePreview; openImage: (src: string, alt: string) => void; download: () => void }) {
  ...
  return <article className="preview-content"><header><strong>{preview.path}</strong><div className="preview-meta">{preview.kind === "text" && <span>{language}</span>}<small>{formatBytes(preview.size)}{preview.mimeType ? ` · ${preview.mimeType}` : ""}</small><button type="button" className="preview-download" onClick={download} title="下载文件" aria-label="下载文件"><Download size={14} aria-hidden="true" /><b>下载</b></button></div></header>{preview.kind === "image" && preview.dataUrl ? <button type="button" className="image-preview" onClick={() => openImage(preview.dataUrl as string, preview.path)} title="点击放大"><img src={preview.dataUrl} alt={preview.path} /><span><ZoomIn size={17} /> 点击放大</span></button> : preview.kind === "text" ? <HighlightedFile content={preview.content ?? ""} language={language} /> : <div className="preview-empty">二进制文件不支持文本预览</div>}</article>;
}
